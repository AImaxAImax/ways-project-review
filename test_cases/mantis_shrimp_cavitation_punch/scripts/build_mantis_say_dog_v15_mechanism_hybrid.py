...
OUT=CASE/'outputs/say_dog_v15_mechanism_hybrid'
...
def mechanism_frame(stage,t):
    # Clean no-text scientific motion plate. No cards, no labels, no thin rings.
    im=Image.new('RGB',(W,H),(5,18,24))
    layer=Image.new('RGBA',(W,H),(0,0,0,0)); d=ImageDraw.Draw(layer,'RGBA')
    # soft water/lab gradient
    for y in range(H):
        a=y/H
        col=(int(5+10*a), int(22+34*(1-a)), int(30+50*(1-a)))
        d.line((0,y,W,y), fill=(*col,255))
    # floating particulate, very low contrast
    rnd=random.Random(902)
    for _ in range(80):
        x=rnd.randrange(W); y=rnd.randrange(H); r=rnd.uniform(1.0,3.2)
        al=rnd.randrange(10,28)
        d.ellipse((x-r,y-r,x+r,y+r), fill=(170,220,230,al))
    cx,cy=585,960
    # shell/prey target as soft rocky oval
    d.ellipse((cx+130,cy-55,cx+355,cy+140), fill=(155,116,82,165))
    d.ellipse((cx+152,cy-40,cx+333,cy+118), fill=(103,78,58,135))
    # club path: smasher appendage as broad orange/yellow curved baton
    p=t
    club_x=cx-320 + 280*min(1,p*1.2)
    club_y=cy+65 - 35*math.sin(p*math.pi)
    d.line((club_x-260,club_y+125,club_x+65,club_y), fill=(255,132,34,190), width=34)
    d.line((club_x-250,club_y+111,club_x+50,club_y-8), fill=(255,221,74,170), width=15)
    # stage-specific bubble/flash, all soft filled shapes
    if stage=='cavitation':
        b=max(0,min(1,(p-.18)/.72)); rr=38+155*b
        al=int(80*(1-abs(b-.58)))
        d.ellipse((cx+65-rr,cy-rr*.62,cx+65+rr,cy+rr*.62), fill=(205,245,255,max(18,al)))
        d.ellipse((cx+65-rr*.62,cy-rr*.36,cx+65+rr*.62,cy+rr*.36), fill=(10,35,48,95))
        d.ellipse((cx+10,cy-70,cx+185,cy+60), fill=(255,255,235,int(28*b)))
    elif stage=='first_hit':
        impact=max(0,1-abs(p-.45)/.22)
        d.ellipse((cx+55-210*impact,cy-90*impact,cx+55+230*impact,cy+90*impact), fill=(255,248,205,int(60*impact)))
        # shock as broad blurred wedge, not ring/line
        d.polygon([(cx+75,cy-85),(cx+455,cy-210),(cx+438,cy+150),(cx+80,cy+80)], fill=(180,235,255,int(42*impact)))
    elif stage=='collapse':
        b=max(0,min(1,p))
        rr=210*(1-b)+34
        d.ellipse((cx+72-rr,cy-rr*.55,cx+72+rr,cy+rr*.55), fill=(160,235,255,int(48*(1-b)+16)))
        pulse=max(0,1-abs(p-.66)/.20)
        d.ellipse((cx+72-270*pulse,cy-125*pulse,cx+72+310*pulse,cy+125*pulse), fill=(255,245,210,int(48*pulse)))
...
def ripple(layer,cx,cy,t,maxr=270,alpha=62,count=2):
    # v15: no geometric rings, they read as artifacts.
    return

def speed_streaks(layer,cx,cy,t):
    # v15: no sharp speed lines, they look synthetic.
    return
...
(OUT/'beat_map_v15.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v15.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v15_silent_1080.mp4'; master=OUT/'mantis_say_dog_v15_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v15_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v15.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v15 clean mechanism hybrid

Status: show candidate if contact-sheet QA passes.

Built after v09 proved generated animal frames were risky. v15 keeps real mantis footage/photos for the animal beats and uses a no-text, full-frame, clean motion-graphic mechanism sequence for cavitation/impact/collapse. No AI animal frames are used.

Outputs:
- Preview: `{preview}`
- Master: `{master}`
- Contact sheet: `{contact}`
- Beat map: `{OUT/'beat_map_v15.json'}`
''')
