...
OUT=CASE/'outputs/say_dog_v19_premium_literal_hybrid'
...
def literal_mech(stage,t):
    # v19: use the polished v17c no-text water/VFX plates as the premium base,
    # then add a restrained project-owned literal club/bubble/target layer so the
    # beat reads as mechanism rather than generic splash art.
    bg_key = 'cavitation' if stage=='cavitation' else ('first_hit' if stage=='first_hit' else 'collapse')
    base=cover(mech_assets[bg_key],.50+.012*math.sin(t*math.pi),.50,1.02+.035*t).convert('RGBA')
    base=ImageEnhance.Contrast(base.convert('RGB')).enhance(1.06).convert('RGBA')
    wash=Image.new('RGBA',(W,H),(0,22,26,42))
    base=Image.alpha_composite(base,wash)
    layer=Image.new('RGBA',(W,H),(0,0,0,0)); d=ImageDraw.Draw(layer,'RGBA')
    cx,cy=560,900
    ang=-0.10
    if stage=='cavitation':
        x=180+485*min(1,t*1.12); y=950-70*math.sin(t*math.pi)
        draw_club(d,x,y,ang,390,74,alpha=178,hot=t>.25)
        bubble_field(layer,x+178,y-8,t,'form')
        glass_target(layer,780,905,t,crack=0)
    elif stage=='first_hit':
        p=min(1,max(0,(t-.12)/.38))
        x=260+415*p; y=955-24*math.sin(t*math.pi)
        draw_club(d,x,y,ang,430,88,alpha=190,hot=True)
        glass_target(layer,790,908,t,crack=max(0,min(1,(t-.22)/.40)))
        bubble_field(layer,758,905,t,'impact')
    else:
        p=min(1,max(0,t/.72))
        x=700+85*p; y=940
        draw_club(d,x,y,ang,390,78,alpha=160,hot=False)
        glass_target(layer,780,908,t,crack=.45+.25*math.sin(t*math.pi))
        bubble_field(layer,612,905,t,'collapse')
        rnd=random.Random(12)
        for i in range(45):
            a=rnd.random()*math.tau; dist=(1-p)*(80+rnd.random()*220)+18
            x1=612+math.cos(a)*dist; y1=905+math.sin(a)*dist*.52
            x2=612+math.cos(a)*(dist-18); y2=905+math.sin(a)*(dist-18)*.52
            d.line((x1,y1,x2,y2),fill=(225,255,255,int(55*(1-p)*(rnd.random()*.7+.3))),width=2)
    layer=layer.filter(ImageFilter.GaussianBlur(.25))
    comp=Image.alpha_composite(base,layer).convert('RGB')
    comp=ImageEnhance.Contrast(comp).enhance(1.07)
...
(OUT/'beat_map_v19.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v19.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v19_silent_1080.mp4'; master=OUT/'mantis_say_dog_v19_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v19_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v19.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v19 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA.

Built after v17 abstract VFX improved polish but failed literal proof. v19 keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v19.json'}`
''')
