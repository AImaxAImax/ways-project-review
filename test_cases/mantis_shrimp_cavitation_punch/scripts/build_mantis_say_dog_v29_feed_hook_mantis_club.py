...
OUT=CASE/'outputs/say_dog_v29_feed_hook_mantis_club'
...
def higgs_plate(t, start=0.0, end=1.0, fx=.50, fy=.50, zoom=1.04):
    # v29: Higgsfield raptorial-club reroll used only as a cinematic source plate.
...
def vertical_water(t,stage='cavitation'):
    # v29: controlled clean underwater macro plate for all mechanism beats.
...
def literal_mech(stage,t):
    # v29: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v29 correction: use one controlled clean mechanism world for all three proof
...
beats=[
 ('01_hook_flash','THIS SHRIMP PUNCH\\nMAKES WATER FLASH',3.92,'higgs','higgs',{'start':.16,'end':.55,'fx':.50,'fy':.50,'zoom':1.00},'higgs_impact',(635,885)),
...
(OUT/'beat_map_v29.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v29.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v29_feed_hook_mantis_club_silent_1080.mp4'; master=OUT/'mantis_say_dog_v29_feed_hook_mantis_club_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v29_feed_hook_mantis_club_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v29_feed_hook_mantis_club.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v29 Feed-hook mantis-club hybrid candidate

Status: internal candidate until contact-sheet/video QA. v29 tests the safest forward route: use a one controlled mantis-club Higgsfield reroll plate plus first-frame hook-first packaging as real-motion mechanism base, reducing the crab-claw read, with only subtle no-text emphasis and creator captions.

Built after the Higgsfield paid-test preflight and QA. v29 keeps real mantis sources for animal/context beats, uses the downloaded silent Higgsfield 480p plate for the cavitation/impact/collapse beats, and adds only subtle no-text bubble/flash/shock emphasis so captions carry the exact biology.
...
- Beat map: `{OUT/'beat_map_v29.json'}`
''')
