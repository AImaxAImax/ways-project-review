...
OUT=CASE/'outputs/say_dog_v18_literal_mechanism'
...
def literal_mech(stage,t):
    ...
    layer=Image.new('RGBA',(W,H),(0,0,0,0)); d=ImageDraw.Draw(layer,'RGBA')
    # Camera composition keeps the exact science object centered away from captions.
    cx,cy=560,900
    ang=-0.10
    if stage=='cavitation':
        x=230+520*min(1,t*1.15); y=960-95*math.sin(t*math.pi)
        draw_club(d,x,y,ang,430,94,alpha=228,hot=t>.25)
        bubble_field(layer,x+185,y-8,t,'form')
        glass_target(layer,780,900,t,crack=0)
    elif stage=='first_hit':
        # club touches the target first; brightest at contact.
        p=min(1,max(0,(t-.12)/.36))
        x=260+430*p; y=950-30*math.sin(t*math.pi)
        draw_club(d,x,y,ang,460,108,alpha=238,hot=True)
        glass_target(layer,790,902,t,crack=max(0,min(1,(t-.24)/.36)))
        bubble_field(layer,760,900,t,'impact')
    else:
        # club is now just past target; detached bubble implodes behind it.
        p=min(1,max(0,t/.72))
        x=720+90*p; y=934
        draw_club(d,x,y,ang,435,96,alpha=210,hot=False)
        glass_target(layer,780,902,t,crack=.55+.30*math.sin(t*math.pi))
        bubble_field(layer,615,900,t,'collapse')
        # inward debris/microjets, organic not diagram arrows.
        rnd=random.Random(12)
        for i in range(55):
            a=rnd.random()*math.tau; dist=(1-p)*(90+rnd.random()*240)+20
            x1=615+math.cos(a)*dist; y1=900+math.sin(a)*dist*.52
            x2=615+math.cos(a)*(dist-22); y2=900+math.sin(a)*(dist-22)*.52
            d.line((x1,y1,x2,y2),fill=(225,255,255,int(80*(1-p)*(rnd.random()*.7+.3))),width=2)
    comp=Image.alpha_composite(base,layer).convert('RGB')
    comp=ImageEnhance.Contrast(comp).enhance(1.08)
    comp=ImageEnhance.Color(comp).enhance(1.05)
    return comp
...
(OUT/'beat_map_v18.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v18.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v18_silent_1080.mp4'; master=OUT/'mantis_say_dog_v18_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v18_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v18.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v18 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA.

Built after v17 abstract VFX improved polish but failed literal proof. v18 keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v18.json'}`
''')
