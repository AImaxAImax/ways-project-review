...
OUT=CASE/'outputs/say_dog_v12_artifact_clean'
...
def ripple(layer,cx,cy,t,maxr=270,alpha=62,count=2):
    ...
    for k in range(count):
        r=maxr*(t*.83+k*.19)
        if 15<r<maxr:
            d.ellipse((cx-r,cy-r*.50,cx+r,cy+r*.50), outline=(216,250,255,int(alpha*.32*(1-r/maxr))), width=1)
...
beats=[
 ...
 ('02_club_speed','ITS CLUB MOVES\\nINSANELY FAST',3.91,'still',5,{'fx':.58,'fy':.48,'zoom0':1.08,'zoom1':1.21,'panx':1},'speed_clean',(585,982)),
 ('03_cavitation','THAT FLASH\\nIS CAVITATION',3.92,'still',2,{'fx':.48,'fy':.52,'zoom0':1.10,'zoom1':1.18,'panx':-1},'cavitation',(615,965)),
 ('04_first_hit','THE PUNCH\\nHITS FIRST',3.91,'still',4,{'fx':.52,'fy':.49,'zoom0':1.12,'zoom1':1.21,'pany':-1},'impact_clean',(585,975)),
...
def make_frame(mode,src,params,kind,loc,t):
    ...
    if kind=='flash':
        flash(layer,cx,cy,t,.26,.58); bubble_cluster(layer,cx,cy,min(1,t*1.1),seed=11,density=8,scale=.36,alpha=30); ripple(layer,cx,cy,max(0,(t-.25)/.75),240,42,2)
        ...
        bubble_cluster(layer,cx,cy,min(1,t*1.05),seed=33,density=16,scale=.42,alpha=38); flash(layer,cx,cy,t,.50,.20)
        ...
        bubble_cluster(layer,cx,cy,t,seed=55,collapse=True,density=18,scale=.44,alpha=42); ripple(layer,cx+10,cy+8,max(0,(t-.42)/.58),310,68,3)
        ...
        bubble_cluster(layer,cx,cy,min(1,t*1.0),seed=88,density=8,scale=.40,alpha=30); bubble_cluster(layer,cx+25,cy+10,max(0,(t-.35)/.65),seed=89,collapse=True,density=14,scale=.44,alpha=40); ripple(layer,cx+25,cy+10,max(0,(t-.35)/.65),345,82,3)
...
(OUT/'beat_map_v12.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v12.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v12_silent_1080.mp4'; master=OUT/'mantis_say_dog_v12_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v12_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v12.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v12 artifact-clean candidate

Status: show candidate if contact-sheet QA passes.

Built after v09 proved generated animal frames were risky. v12 returns to the real-source backbone and removes the artifact-prone mechanics cards/geometric rings. It uses only subtle soft flash and particle cues. No AI animal frames are used.

Outputs:
- Preview: `{preview}`
- Master: `{master}`
- Contact sheet: `{contact}`
- Beat map: `{OUT/'beat_map_v12.json'}`
''')
