...
OUT=CASE/'outputs/say_dog_v28_mantis_club_reroll_hybrid'
...
def higgs_plate(t, start=0.0, end=1.0, fx=.50, fy=.50, zoom=1.04):
    # v28: Higgsfield raptorial-club reroll used only as a cinematic source plate.
...
def vertical_water(t,stage='cavitation'):
    # v28: controlled clean underwater macro plate for all mechanism beats.
...
def literal_mech(stage,t):
    # v28: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v28 correction: use one controlled clean mechanism world for all three proof
...
(OUT/'beat_map_v28.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v28.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v28_mantis_club_reroll_hybrid_silent_1080.mp4'; master=OUT/'mantis_say_dog_v28_mantis_club_reroll_hybrid_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v28_mantis_club_reroll_hybrid_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v28_mantis_club_reroll_hybrid.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v28 Higgsfield mantis-club reroll hybrid candidate

Status: internal candidate until contact-sheet/video QA. v28 tests the safest forward route: use a one controlled mantis-club Higgsfield reroll plate as real-motion mechanism base, reducing the crab-claw read, with only subtle no-text emphasis and creator captions.

Built after the Higgsfield paid-test preflight and QA. v28 keeps real mantis sources for animal/context beats, uses the downloaded silent Higgsfield 480p plate for the cavitation/impact/collapse beats, and adds only subtle no-text bubble/flash/shock emphasis so captions carry the exact biology.
...
- Beat map: `{OUT/'beat_map_v28.json'}`
''')
