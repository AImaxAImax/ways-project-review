...
OUT=CASE/'outputs/say_dog_v23_clean_3d_mechanism'
...
def vertical_water(t,stage='cavitation'):
    # v23: controlled clean underwater macro plate for all mechanism beats.
...
def bubble_field(layer,cx,cy,t,mode):
    ...
    if mode=='form':
        ...
        for k in range(5,0,-1):
            rr=r*(.62+k*.075); a=int(7*k*(1-p*.20))
            d.ellipse((cx-rr*1.10,cy-rr*.66,cx+rr*1.10,cy+rr*.66),outline=(205,248,255,a),width=3)
        d.ellipse((cx-r*.48,cy-r*.26,cx+r*.48,cy+r*.26),fill=(218,252,255,int(28+26*p)),outline=(245,255,255,96),width=2)
    elif mode=='impact':
...
def literal_mech(stage,t):
    # v23: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v23 correction: use one controlled clean mechanism world for all three proof
    ...
    if stage=='cavitation':
        ...
        for j,a in enumerate([30,18]):
            draw_club(d,x-62*(j+1),y+6*(j+1),ang,420,78,alpha=a,hot=False)
        draw_club(d,x,y,ang,455,96,alpha=220,hot=t>.20)
...
(OUT/'beat_map_v23.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v23.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v23_silent_1080.mp4'; master=OUT/'mantis_say_dog_v23_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v23_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v23.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v23 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA. v23 tests whether a consistent clean 3D-style mechanism world beats the v21/v22 mixed premium/procedural backgrounds.

Built after v17 abstract VFX improved polish but failed literal proof. v23 artifact-clean pass keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v23.json'}`
''')
