...
OUT=CASE/'outputs/say_dog_v25_mechanism_closeup'
...
def vertical_water(t,stage='cavitation'):
    # v25: controlled clean underwater macro plate for all mechanism beats.
...
def literal_mech(stage,t):
    # v25: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v25 correction: use one controlled clean mechanism world for all three proof
...
(OUT/'beat_map_v25.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v25.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v25_silent_1080.mp4'; master=OUT/'mantis_say_dog_v25_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v25_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v25.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v25 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA. v25 tests whether a consistent clean 3D-style mechanism world beats the v21/v22 mixed premium/procedural backgrounds.

Built after v17 abstract VFX improved polish but failed literal proof. v25 artifact-clean pass keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v25.json'}`
''')
