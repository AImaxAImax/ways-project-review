...
OUT=CASE/'outputs/say_dog_v13_fullres_clean'
...
def make_frame(mode,src,params,kind,loc,t):
    ...
    if kind=='flash':
        ...
        flash(layer,cx+10,cy+8,t,.55,.08)
        ...
        flash(layer,cx+25,cy+10,t,.45,.10)
...
(OUT/'beat_map_v13.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v13.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v13_silent_1080.mp4'; master=OUT/'mantis_say_dog_v13_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v13_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v13.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v13 full-res artifact-clean candidate

Status: show candidate if contact-sheet QA passes.

Built after v09 proved generated animal frames were risky. v13 removes artifact-prone cards/rings/particle clusters and uses full-resolution stills where available, with only soft flash cues. No AI animal frames are used.

Outputs:
- Preview: `{preview}`
- Master: `{master}`
- Contact sheet: `{contact}`
- Beat map: `{OUT/'beat_map_v13.json'}`
''')
