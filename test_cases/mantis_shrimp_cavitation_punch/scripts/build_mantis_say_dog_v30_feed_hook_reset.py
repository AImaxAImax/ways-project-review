...
OUT=CASE/'outputs/say_dog_v30_feed_hook_reset'
...
for name,path in {'peacock':VIDEO_DIR/'peacock_mantis.webm','wide':VIDEO_DIR/'mantis_philippines.webm','higgs':CASE/'outputs/higgsfield_video_route/higgsfield_mantis_club_reroll02_480p_silent.mp4'}.items():
...
def higgs_plate(t, start=0.0, end=1.0, fx=.50, fy=.50, zoom=1.04):
    # v30: Higgsfield raptorial-club reroll used only as a cinematic source plate.
...
def vertical_water(t,stage='cavitation'):
    # v30: controlled clean underwater macro plate for all mechanism beats.
...
def literal_mech(stage,t):
    # v30: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v30 correction: use one controlled clean mechanism world for all three proof
...
beats=[
 ('01_hook_flash','THIS SHRIMP PUNCH\\nMAKES WATER FLASH',3.92,'hook_feed','hook',{},'higgs_impact',(635,885)),
 ('02_club_speed','ITS CLUB MOVES\\nINSANELY FAST',3.91,'still',5,{'fx':.58,'fy':.48,'zoom0':1.02,'zoom1':1.10,'panx':1},'speed_clean',(585,982)),
 ('03_cavitation','THAT FLASH\\nIS CAVITATION',3.92,'higgs','higgs',{'start':.08,'end':.42,'fx':.50,'fy':.50,'zoom':1.02},'higgs_bubble',(620,880)),
 ('04_first_hit','THE PUNCH\\nHITS FIRST',3.91,'higgs','higgs',{'start':.18,'end':.55,'fx':.50,'fy':.50,'zoom':1.02},'higgs_impact',(635,885)),
 ('05_second_hit','THEN THE BUBBLE\\nCOLLAPSES',3.91,'higgs','higgs',{'start':.34,'end':.92,'fx':.50,'fy':.50,'zoom':1.02},'higgs_collapse',(630,890)),
...
def base_frame(mode,src,params,t):
    if mode=='hook_feed':
        # Analytics pass: first 0.5-1.5s is proof/contradiction visual,
        # then reset before second 3 to avoid a static opening hold.
        if t < .52:
            return higgs_plate(t/.52, start=.16, end=.45, fx=.50, fy=.50, zoom=1.00)
        return video('peacock', (t-.52)/.48, fx=.31, zoom=1.06, offset=0, speed=.92)
...
'''
    ...
    for bid,cap,dur,mode,*_ in beats:
        ...
        pos='{\\pos(540,500)}' if mode in ('mech','higgs') else ''
...
(OUT/'beat_map_v30.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v30.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v30_feed_hook_reset_silent_1080.mp4'; master=OUT/'mantis_say_dog_v30_feed_hook_reset_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v30_feed_hook_reset_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v30_feed_hook_reset.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v30 Feed-hook mantis-club hybrid candidate

Status: internal candidate until contact-sheet/video QA. v30 tests the safest forward route: use a one controlled mantis-club Higgsfield reroll plate plus first-frame hook-first packaging as real-motion mechanism base, reducing the crab-claw read, with only subtle no-text emphasis and creator captions.

Built after the Higgsfield paid-test preflight and QA. v30 keeps real mantis sources for animal/context beats, uses the downloaded silent Higgsfield 480p plate for the cavitation/impact/collapse beats, and adds only subtle no-text bubble/flash/shock emphasis so captions carry the exact biology.
...
- Beat map: `{OUT/'beat_map_v30.json'}`
''')
