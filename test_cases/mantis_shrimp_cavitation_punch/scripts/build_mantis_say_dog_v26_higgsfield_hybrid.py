...
OUT=CASE/'outputs/say_dog_v26_higgsfield_hybrid'
...
def higgs_plate(t, start=0.0, end=1.0, fx=.50, fy=.50, zoom=1.04):
    # v26: Higgsfield paid-test result used only as a cinematic source plate.
...
def vertical_water(t,stage='cavitation'):
    # v26: controlled clean underwater macro plate for all mechanism beats.
...
def literal_mech(stage,t):
    # v26: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v26 correction: use one controlled clean mechanism world for all three proof
...
beats=[
 ...
 ('03_cavitation','THAT FLASH\\nIS CAVITATION',3.92,'higgs','higgs',{'start':.05,'end':.45,'fx':.52,'fy':.50,'zoom':1.03},'higgs_bubble',(655,900)),
 ('04_first_hit','THE PUNCH\\nHITS FIRST',3.91,'higgs','higgs',{'start':.28,'end':.70,'fx':.52,'fy':.50,'zoom':1.06},'higgs_impact',(730,930)),
 ('05_second_hit','THEN THE BUBBLE\\nCOLLAPSES',3.91,'higgs','higgs',{'start':.55,'end':.98,'fx':.52,'fy':.50,'zoom':1.08},'higgs_collapse',(640,895)),
...
(OUT/'beat_map_v26.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v26.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v26_higgsfield_hybrid_silent_1080.mp4'; master=OUT/'mantis_say_dog_v26_higgsfield_hybrid_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v26_higgsfield_hybrid_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v26_higgsfield_hybrid.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v26 Higgsfield hybrid candidate

Status: internal candidate until contact-sheet/video QA. v26 tests the safest forward route: use the verified one-credit-controlled Higgsfield plate as real-motion mechanism base, with only subtle no-text emphasis and creator captions.

Built after the Higgsfield paid-test preflight and QA. v26 keeps real mantis sources for animal/context beats, uses the downloaded silent Higgsfield 480p plate for the cavitation/impact/collapse beats, and adds only subtle no-text bubble/flash/shock emphasis so captions carry the exact biology.
...
- Beat map: `{OUT/'beat_map_v26.json'}`
''')
