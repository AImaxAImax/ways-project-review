...
OUT=CASE/'outputs/say_dog_v22_artifact_clean_flash'
...
def literal_mech(stage,t):
    # v22: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    if stage == 'cavitation':
        # v22 correction: the generated/premium flash plate read as artifacted under
        # the caption. Use a controlled procedural water plate for this beat so
        # the only bright event is the intentional vapor-bubble flash.
        base = vertical_water(t, 'cavitation').convert('RGBA')
        base = ImageEnhance.Contrast(base.convert('RGB')).enhance(0.94).convert('RGBA')
        base = Image.alpha_composite(base, Image.new('RGBA',(W,H),(0,16,22,38)))
    else:
    ...
    pd.ellipse((120,610,1015,1190),fill=(0,10,12,104 if stage == 'cavitation' else 112))
    pd.ellipse((250,710,935,1095),fill=(0,0,0,64 if stage == 'cavitation' else 76))
...
(OUT/'beat_map_v22.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v22.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v22_silent_1080.mp4'; master=OUT/'mantis_say_dog_v22_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v22_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v22.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v22 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA.

Built after v17 abstract VFX improved polish but failed literal proof. v22 artifact-clean pass keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v22.json'}`
''')
