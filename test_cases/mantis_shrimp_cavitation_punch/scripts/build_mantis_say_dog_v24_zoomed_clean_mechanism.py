...
OUT=CASE/'outputs/say_dog_v24_zoomed_clean_mechanism'
...
def vertical_water(t,stage='cavitation'):
    # v24: controlled clean underwater macro plate for all mechanism beats.
...
def draw_club(draw, cx, cy, angle, length, width, alpha=235, hot=False):
    ...
    draw.polygon(pts,fill=(190,76,34,alpha),outline=(250,176,86,min(180,alpha)))
    ...
    draw.polygon(inner,fill=(229,116,43,int(alpha*.78)))
    ...
    draw.polygon(stripe,fill=(255,205,96,96 if not hot else 140))
...
def glass_target(layer,cx,cy,t,crack=0.0):
    ...
    d.ellipse((cx-178,cy-86,cx+178,cy+86),fill=(42,92,91,154),outline=(190,238,232,96),width=3)
    for k in range(5):
        yy=cy-54+k*24
        d.arc((cx-148,yy-38,cx+148,yy+38),205,335,fill=(244,255,235,42+k*7),width=2)
    d.ellipse((cx-58,cy-28,cx+56,cy+28),fill=(231,245,221,42),outline=(230,255,250,48),width=1)
    if crack>0:
...
def literal_mech(stage,t):
    # v24: same premium VFX base as v19, but the mechanism layer is larger,
    ...
    # v24 correction: use one controlled clean mechanism world for all three proof
    ...
    base = Image.alpha_composite(base, Image.new('RGBA',(W,H),(0,16,22,36)))
    ...
    pd.ellipse((108,612,1028,1192),fill=(0,10,12,104))
    pd.ellipse((250,710,935,1095),fill=(0,0,0,60))
    ...
    if stage=='cavitation':
        p=min(1,t*1.08)
        x=64+690*p; y=982-78*math.sin(t*math.pi)
        # Phone-size zoom: larger club and bubble, less faint ghosting.
        for j,a in enumerate([34,18]):
            draw_club(d,x-86*(j+1),y+8*(j+1),ang,560,108,alpha=a,hot=False)
        draw_club(d,x,y,ang,650,142,alpha=228,hot=t>.20)
        bubble_field(layer,x+278,y-8,t,'form')
        glass_target(layer,855,934,t,crack=0)
        # soft pressure pocket behind the forming bubble
        pp=min(1,max(0,(t-.18)/.55))
        if pp:
            for k in range(4,0,-1):
                rr=90+k*58*pp
                d.ellipse((x+268-rr*1.15,y-rr*.52,x+268+rr*1.15,y+rr*.52),fill=(210,250,255,int(7*k*pp)))
    elif stage=='first_hit':
        p=min(1,max(0,(t-.10)/.34))
        x=130+625*p; y=984-24*math.sin(t*math.pi)
        draw_club(d,x-78,y+7,ang,590,116,alpha=54,hot=False)
        draw_club(d,x,y,ang,690,154,alpha=242,hot=True)
        glass_target(layer,858,936,t,crack=max(0,min(1,(t-.20)/.35)))
        bubble_field(layer,830,934,t,'impact')
        # contact spark, organic bloom not a geometric callout.
        pp=max(0,1-abs(t-.34)/.18)
        if pp:
            for k in range(6,0,-1):
                rr=42+k*45
                d.ellipse((824-rr,934-rr*.34,824+rr,934+rr*.34),fill=(255,246,190,int(14*k*pp)))
    else:
        p=min(1,max(0,t/.70))
        x=704+82*p; y=974
        draw_club(d,x,y,ang,640,132,alpha=205,hot=False)
        glass_target(layer,858,938,t,crack=.64+.24*math.sin(t*math.pi))
        bubble_field(layer,585,934,t,'collapse')
        # Extra crisp shrinking core so 'collapse' reads at phone size.
        core=max(0,1-abs(t-.56)/.28)
        if core:
            d.ellipse((585-76*core,934-48*core,585+76*core,934+48*core),fill=(255,255,238,int(150*core)))
        rnd=random.Random(12)
        for i in range(62):
            a=rnd.random()*math.tau; dist=(1-p)*(120+rnd.random()*315)+20
            x1=585+math.cos(a)*dist; y1=934+math.sin(a)*dist*.52
            x2=585+math.cos(a)*(dist-26); y2=934+math.sin(a)*(dist-26)*.52
            d.line((x1,y1,x2,y2),fill=(225,255,255,int(76*(1-p)*(rnd.random()*.7+.3))),width=2)
...
(OUT/'beat_map_v24.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v24.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v24_silent_1080.mp4'; master=OUT/'mantis_say_dog_v24_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v24_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v24.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v24 literal mechanism candidate

Status: internal candidate until contact-sheet/video QA. v24 tests whether a consistent clean 3D-style mechanism world beats the v21/v22 mixed premium/procedural backgrounds.

Built after v17 abstract VFX improved polish but failed literal proof. v24 artifact-clean pass keeps real mantis sources for animal/context beats and replaces abstract mechanism shots with project-owned no-text literal mechanism animation: stylized raptorial club, hard target, cavitation bubble growth, first impact flash, and bubble collapse/shock glow.
...
- Beat map: `{OUT/'beat_map_v24.json'}`
''')
