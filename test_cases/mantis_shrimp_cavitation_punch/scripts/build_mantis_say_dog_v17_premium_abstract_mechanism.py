...
OUT=CASE/'outputs/say_dog_v17_premium_abstract_mechanism'
...
def premium_mech(name,t,zoom0=1.00,zoom1=1.06,panx=0,pany=0):
    im=mech_assets[name]
    z=zoom0+(zoom1-zoom0)*t
    fx=.5+panx*math.sin(t*math.pi)*.018
    fy=.5+pany*math.sin(t*math.pi)*.018
    # lighter grade to preserve generated highlight detail
    out=cover(im,fx,fy,z)
...
def ripple(layer,cx,cy,t,maxr=270,alpha=62,count=2):
    # v17: no geometric rings, they read as artifacts.
    return

def speed_streaks(layer,cx,cy,t):
    # v17: no sharp speed lines, they look synthetic.
    return
...
beats=[
 ...
 ('03_cavitation','THAT FLASH\\nIS CAVITATION',3.92,'mech','cavitation',{'zoom0':1.00,'zoom1':1.06,'pany':-1},'clean',(615,965)),
 ('04_first_hit','THE PUNCH\\nHITS FIRST',3.91,'mech','first_hit',{'zoom0':1.02,'zoom1':1.10,'panx':1},'clean',(585,975)),
 ('05_second_hit','THEN THE BUBBLE\\nCOLLAPSES',3.91,'mech','collapse',{'zoom0':1.00,'zoom1':1.07,'panx':-1},'clean',(575,990)),
...
def base_frame(mode,src,params,t):
    ...
    if mode=='mech': return premium_mech(src,t,**params)
...
(OUT/'beat_map_v17.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v17.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v17_silent_1080.mp4'; master=OUT/'mantis_say_dog_v17_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v17_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v17.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v17 premium abstract mechanism candidate

Status: show candidate if contact-sheet QA passes.

Built after v09 proved generated animal frames were risky. v17 uses real mantis sources for animal beats and selected no-text abstract premium mechanism frames for cavitation/impact/collapse. No generated animal frames are used.

Outputs:
- Preview: `{preview}`
- Master: `{master}`
- Contact sheet: `{contact}`
- Beat map: `{OUT/'beat_map_v17.json'}`
''')
