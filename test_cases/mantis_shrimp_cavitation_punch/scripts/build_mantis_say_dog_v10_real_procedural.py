...
OUT=CASE/'outputs/say_dog_v10_real_procedural'
...
MECH=CASE/'assets/source_commons_mechanics/smashing_en.gif'
...
mech_dir=SRC/'mechanics'; mech_dir.mkdir(parents=True,exist_ok=True)
subprocess.run(['ffmpeg','-y','-v','error','-i',str(MECH),'-vf','fps=24',str(mech_dir/'f_%05d.png')],check=True)
...
mech_frames=[Image.open(p).convert('RGB') for p in sorted(mech_dir.glob('f_*.png'))]
...
def mech_inset(t,large=False):
    # Crop only the no-label/no-title diagram region from the Commons mechanics GIF.
    src=mech_frames[int(t*(len(mech_frames)-1))%len(mech_frames)]
    crop=src.crop((360,86,970,500))
    crop=ImageOps.autocontrast(crop)
    crop=ImageEnhance.Contrast(crop).enhance(1.12)
    target_w=720 if large else 540
    target_h=int(crop.height*target_w/crop.width)
    crop=crop.resize((target_w,target_h),Image.LANCZOS).convert('RGBA')
    # Convert white background to transparent-ish frosted card.
    px=crop.load()
    for y in range(crop.height):
        for x in range(crop.width):
            r,g,b,a=px[x,y]
            if r>245 and g>245 and b>245:
                px[x,y]=(245,252,255,18)
            elif r>225 and g>225 and b>225:
                px[x,y]=(235,248,255,38)
            else:
                px[x,y]=(r,g,b,220)
    pad=24
    card=Image.new('RGBA',(crop.width+pad*2,crop.height+pad*2),(0,0,0,0))
    shadow=Image.new('RGBA',card.size,(0,0,0,0)); sd=ImageDraw.Draw(shadow,'RGBA')
    sd.rounded_rectangle((4,8,card.width-4,card.height-4),radius=34,fill=(0,0,0,72))
    shadow=shadow.filter(ImageFilter.GaussianBlur(16)); card=Image.alpha_composite(card,shadow)
    cd=ImageDraw.Draw(card,'RGBA')
    cd.rounded_rectangle((0,0,card.width-10,card.height-12),radius=32,fill=(230,248,255,34),outline=(210,250,255,80),width=2)
    card.alpha_composite(crop,(pad,pad))
    return card
...
for bi,(bid,cap,dur,mode,src,params,kind,loc) in enumerate(beats,1):
    n=round(dur*FPS)
    bm.append({'beat':bi,'id':bid,'caption':cap.replace('\\n',' / '),'mode':mode,'source':str(src),'visual_kind':kind,'frames':n,'strategy':'real video/photos + procedural bubbles/flash/ripples; cropped no-label Commons mechanics inset; no AI animal frames'})
    for f in range(n):
        make_frame(mode,src,params,kind,loc,f/max(1,n-1)).save(FRAMES/f'frame_{idx:05d}.jpg',quality=94); idx+=1
(OUT/'beat_map_v10.json').write_text(json.dumps(bm,indent=2))
ass=OUT/'captions_say_dog_v10.ass'; write_ass(ass)
silent=OUT/'mantis_say_dog_v10_silent_1080.mp4'; master=OUT/'mantis_say_dog_v10_captioned_1080.mp4'; preview=OUT/'mantis_say_dog_v10_captioned_720.mp4'; contact=OUT/'contact_sheet_mantis_say_dog_v10.jpg'
...
(OUT/'README_QA.md').write_text(f'''# Mantis shrimp cavitation punch — v10 real/procedural candidate

Status: show candidate if contact-sheet QA passes.

Built after v09 proved generated animal frames were risky. v10 returns to v08's real-source backbone and adds only controlled, no-text procedural bubbles/flash/ripples plus a cropped no-label Wikimedia mechanics inset. No AI animal frames are used.

Outputs:
- Preview: `{preview}`
- Master: `{master}`
- Contact sheet: `{contact}`
- Beat map: `{OUT/'beat_map_v10.json'}`
''')
