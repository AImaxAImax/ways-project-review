...
OUT = CASE / 'outputs/motion_v08_no_artifact_transition'
FRAMES = OUT / 'frames'
...
def scene_frame(scene: str, u: float):
    ...
    if scene == 'thaw':
        # v08: no split-screen proof plate and no translucent oval. Crossfade real cold frog to real warm frog
        # with warm light sweep for motion, keeping frames artifact-clean.
        cold = base_motion(ASSETS['frog2'], min(1,u*1.25), fx=.52, fy=.48, zoom0=1.07, zoom1=1.14, panx=.010, cold=True)
        warm = base_motion(ASSETS['frog4'], max(0,(u-.18)/.82), fx=.52, fy=.48, zoom0=1.08, zoom1=1.18, panx=-.018, warm=True)
        alpha = min(1,max(0,(u-.18)/.48))
        fr = Image.blend(cold, warm, alpha)
        return thaw_warmth(fr, .25+.75*u)
    if scene == 'final':
...
# silent clean base
silent = OUT/'wood_frog_motion_v08_clean_1080.mp4'
...
master = OUT/'wood_frog_motion_v08_captioned_1080.mp4'
...
preview = OUT/'wood_frog_motion_v08_captioned_720.mp4'
...
sheet = OUT/'contact_sheet_wood_frog_motion_v08.jpg'
...
manifest = {
    ...
    'version':'v08_no_artifact_transition_static_motion',
