...
OUT = CASE / 'outputs/static_motion_v06_artifact_clean'
FRAMES = OUT / 'frames'
...
def scene_frame(scene: str, u: float):
    if scene == 'hook':
        fr = base_motion(ASSETS['hook'], u, zoom0=1.0, zoom1=1.09, cold=True)
        return frost_creep(fr, u, 10, .75)
    if scene == 'freeze_body':
        fr = base_motion(ASSETS['freeze'], u, zoom0=1.04, zoom1=1.12, panx=.015, cold=True)
        return frost_creep(fr, u, 22, .9)
    if scene == 'heart_stop':
        fr = base_motion(ASSETS['heart'], u, zoom0=1.0, zoom1=1.08, cold=True)
        fr = frost_creep(fr, u, 33, .55)
...
def caption_ass():
    ...
    lines = [
        ...
        'Style: Caption,DejaVu Sans Condensed,108,&H00FFFFFF,&H0000D7FF,&H00000000,&H80000000,-1,0,0,0,100,100,-1,0,1,9,4,5,70,70,690,1',
        '', '[Events]',
...
# silent clean base
silent = OUT/'wood_frog_static_motion_v06_clean_1080.mp4'
...
master = OUT/'wood_frog_static_motion_v06_captioned_1080.mp4'
...
preview = OUT/'wood_frog_static_motion_v06_captioned_720.mp4'
...
sheet = OUT/'contact_sheet_wood_frog_static_motion_v06.jpg'
...
manifest = {
    ...
    'version':'v06_artifact_clean_static_motion',
    'motion_policy':'deterministic source-preserving static motion: zoom/pan/frost/thaw/cell overlays; no AI-generated frog bodies; no paid video credits',
