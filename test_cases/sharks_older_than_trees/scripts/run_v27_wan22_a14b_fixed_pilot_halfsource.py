...
prep=OUT/'shot07_source_216x384.png'
im=Image.open(SHOT['path']).convert('RGB')
w,h=im.size
target=432/768
if w/h > target:
    new_w=int(h*target); x=(w-new_w)//2; im=im.crop((x,0,x+new_w,h))
else:
    new_h=int(w/target); y=(h-new_h)//2; im=im.crop((0,y,w,y+new_h))
im=im.resize((216,384), Image.Resampling.LANCZOS)
im.save(prep)
name=upload(prep)
